...
public class Asesores extends JFrame implements ActionListener {
    ...
    public JLabel lCuenta = new JLabel("Id Asesor: ");
    ...
    public JLabel lAtendidos = new JLabel("Alumnos Atendidos: ");
    ...
    public Asesores() {
        ...
        lAsesoria.setBounds(100, 330, 100, 40);
        lAsesoria.setForeground(Color.WHITE);
        lHorario.setBounds(100, 380, 100, 40);
        lHorario.setForeground(Color.WHITE);
        lSalon.setBounds(100, 430, 150, 40);
        lSalon.setForeground(Color.WHITE);
        lAtendidos.setBounds(100, 480, 150, 40);
        lAtendidos.setForeground(Color.WHITE);
        ...
        t6.setBounds(250, 435, 170, 25);
        t7.setBounds(250, 485, 170, 25);
    ...
    public void actionPerformed(ActionEvent e) {
        ...
        if (p.equals("Consultar")) {
            ...
            try {
                ...
                ps = conn.prepareStatement("SELECT id_asesor,nombres,apellidos,asesorias,horario,salon,alumnos_atendidos  FROM asesores WHERE id_asesor = ?");
                ...
                if (rs.next()) {
                    t1.setText(rs.getString("id_asesor"));
                    t2.setText(rs.getString("nombres"));
                    t3.setText(rs.getString("apellidos"));
                    t4.setText(rs.getString("asesorias"));
                    t5.setText(rs.getString("horario"));
                    t6.setText(rs.getString("salon"));
                    t7.setText(rs.getString("alumnos_atendidos"));

                } else {
                    JOptionPane.showMessageDialog(null, "No se encontro ningun asesor con ese Id");
                }
            ...
            try {
                ...
                ps = conn.prepareStatement("INSERT INTO asesores (id_asesor,nombres, apellidos,asesorias,horario,salon,alumnos_atendidos) VALUES(?,?,?,?,?,?,?)");
                ...
                int eleccion = JOptionPane.showOptionDialog(this, "Realmente desea Registrar Asesor?", p, 0, 0, null, botones, this);
            ...
            try {
                conn = Conexion.getConnnection();
                ps = conn.prepareStatement("UPDATE asesores SET id_asesor =?,nombres =?,apellidos =?,asesorias =?,horario =?,salon =?,alumnos_atendidos =? WHERE id_asesor = ? ");
...
//                ps.setString(6, t1.getText());
                String botones[] = {"si", "no"};
                int eleccion = JOptionPane.showOptionDialog(this, "Realmente desea Modificar Asesor?", p, 0, 0, null, botones, this);
                if (eleccion == JOptionPane.YES_OPTION) {
                    ps.executeUpdate();
            ...
            try {
                conn = Conexion.getConnnection();
                ps = conn.prepareStatement("DELETE FROM asesores WHERE id_asesor = ?");

                ps.setString(1, t1.getText());

                String botones[] = {"si", "no"};
                int eleccion = JOptionPane.showOptionDialog(this, "Realmente desea Eliminar Asesor?", p, 0, 0, null, botones, this);
                if (eleccion == JOptionPane.YES_OPTION) {
                    ps.executeUpdate();
            ...
            try {
                ...
                rs = s.executeQuery("select id_asesor,nombres,apellidos,asesorias,horario,salon,alumnos_atendidos from asesores");
            ...
            try {
                ...
                PdfWriter.getInstance(documento, new FileOutputStream(ruta + "/Desktop/Reporte_Asesores.pdf"));
                
                Paragraph parrafo = new Paragraph();
                parrafo.setAlignment(Paragraph.ALIGN_CENTER);
                parrafo.add("Formato de asesores © \n\n");
                parrafo.setFont(FontFactory.getFont("Arial", 8, Font.BOLD, BaseColor.DARK_GRAY));
                parrafo.add("LISTADO DE ASESORES \n\n");
                ...
                tabla.addCell("id_asesor");
                tabla.addCell("nombres");
                tabla.addCell("apellidos");
                tabla.addCell("asesorias");
                tabla.addCell("horaio");
                tabla.addCell("salon");
                tabla.addCell("alumnos_atendidos");

                try {
                    Connection conn = DriverManager.getConnection("jdbc:oracle:thin:@localhost:1521:XE", "system", "system");
                    PreparedStatement pst = conn.prepareStatement("select * from asesores");
